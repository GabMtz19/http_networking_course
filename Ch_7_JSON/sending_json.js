async function updateLocationById(id, locationObj, apiKey) {
...
async function main() {
    ...
    const location = await getLocationById(locationID, apiKey)
    console.log(`Location '${location.name}' fetched. Data: ${JSON.stringify(location)}`)
    
    location.discovered = true
    await updateLocationById(locationID, location, apiKey)
    console.log(`Location '${location.name}' was discovered!`)
    
    const updatedLocation = await getLocationById(locationID, apiKey)
  ...
  async function getLocationById(id, apiKey) {
    const path = `https://api.boot.dev/v1/courses_rest_api/learn-http/locations/${id}`
    const response = await fetch(path, {
      method: 'GET',
      mode: 'cors',
