async function lootTreasure(baseURL, chestRarity, apiKey) {
    let limit = null;
    switch (chestRarity) {
        case 'Common':
            limit = 1;
...
async function main() {
    ...
    const commonLoot = await lootTreasure(url, 'Common', apiKey)
    ...
    const rareLoot = await lootTreasure(url, 'Rare', apiKey)
    ...
    const legendaryLoot = await lootTreasure(url, 'Legendary', apiKey)
